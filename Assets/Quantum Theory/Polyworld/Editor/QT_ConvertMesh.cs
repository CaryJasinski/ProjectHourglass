 ...
 * - as a result, gameobjects with skinnesmeshrendere components that have blendshapes but no boneweights get converted to simple props (meshrenderer and meshfilter component-based). 
...
public class QT_ConvertMesh : EditorWindow
{
  ...
  //  bool combineMats = true;
        ...
        if (Selection.gameObjects.Length > 0)
        {
            ...
            else
            {
                ...
                if (GUILayout.Button("Convert to PolyWorld Mesh"))
                {
                    ...
                    List<string> BadChars = CheckBadCharacters(SourceGOs);
                    ...
                    else if (BadChars.Count > 0)
                    {
                        EditorUtility.DisplayDialog("Bad Characters Found", "Illegal characters were found in the names of meshes within the selection.\n\nCheck the Console for the list of Gameobjects with invalid mesh names.\n\nAborting.", "OK");
    ...
    private List<string> CheckBadCharacters(GameObject[] SourceGOs)
    {
        List<string> badMeshes = new List<string>();
        string[] badChars = { "#","%","&","{","}","\\","<",">","*","?","/","$","!","'",":","@","+","`","|","=" };
        ...
        foreach (GameObject g in SourceGOs) //for every GO you have selected
        {
           // MeshRenderer[] mrs = g.GetComponentsInChildren<MeshRenderer>(true);
            ...
            foreach (MeshFilter mf in mfs)
            {
                ...
                {
                    if (mf.sharedMesh.name.Contains(c))
                    {
                        badMeshes.Add(mf.sharedMesh.name);
                        Debug.LogError("Mesh named: " + mf.sharedMesh.name + " on GameObject: " + mf.gameObject.name + "   contains illegal characters.\nPlease rename the mesh inside your 3d application and reexport.");
                    }
                }
            ...
            foreach (SkinnedMeshRenderer smr in smrs)
            {
                foreach (string c in badChars)
                {
                    if (smr.sharedMesh.name.Contains(c))
                    {
                        badMeshes.Add(smr.sharedMesh.name);
                        Debug.LogError("Mesh named: " + smr.sharedMesh.name + " on GameObject: "+smr.gameObject.name+"   contains illegal characters.\nPlease rename the mesh inside your 3d application and reexport.");
                    }
                }
            }
        }
   ...
   // private List<string> FinalBSs = new List<string>(); //blendshapes?
   // private List<float> FinalBSWeights = new List<float>(); //blendshape weights
